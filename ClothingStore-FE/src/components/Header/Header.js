...
export default function Header() {
    ...
    const handleLogin = () => {
        if (userLogin !== 'true') {
            ...
            return (
                <>
                    <button onClick={() => navigate('/account')} className="dropdown-item">Account</button>
                    <button onClick={() => navigate('/order')} className="dropdown-item">Order</button>
                    <button onClick={() => handleClicker()} className="dropdown-item">Log out</button>
                </>
            )
        }
    }
    ...
    const handleSearch = (e) => {
        if (e !== "") {
            axios({
                ...
                console.log("Unable to get type :", err)
            })
            setShowDrop("block")
        } else {
            setShowDrop("")
                ...
                <div className="container-fluid">
                    <div className="row align-items-center bg-light py-3 px-xl-5 d-none d-lg-flex">
                                ...
                                {arrProdcutSearch.map((item, index) => {
                                    return (
                                        <li style={{ marginBottom: "15px" }}>
                                            ...
                                            <a href={`http://localhost:3001/product/${item.maSP}`} style={{ color: "black" }}>{item.tenSP}</a>
                                        </li>
                                    )
                                })}
                            </ul>
                        ...
                        <div className="col-lg-4 col-6 text-right">
                            <p className="m-0">CONTACT SUPPORT ZALO</p>
                            <h5 className="m-0">+84 933 155 421</h5>
                        </div>
                    </div>
                </div>
                {/* Topbar End */}
                            ...
                            <nav className="navbar navbar-expand-lg bg-dark navbar-dark py-3 py-lg-0 px-0">
                                ...
                                <div className="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                                    <div className="navbar-nav mr-auto py-0">
                                        ...
                                        <NavLink to='/shop' className="nav-item nav-link">Product</NavLink>
                                    ...
                                    <div className="navbar-nav ml-auto py-0 d-none d-lg-block">
                                        ...
                                        <div className="d-inline-flex align-items-center">
                                            <div className="btn-group">
                                                <button type="button" className="btn btn-sm btn-light dropdown-toggle" data-toggle="dropdown">{userLogin ? `Hi ${JSON.parse(localStorage.getItem("infoUser")).hoTen}` : "Account"}</button>
