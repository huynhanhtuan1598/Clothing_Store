...
export default function Index() {
    ...
    const deleteSale = async (id) => {
        if (window.confirm("You Want To Remove Promotion?") === true) {
            await axios({
                method: 'delete',
        ...
        try {
            arrSize.forEach(item => {
                axios({
                    ...
                    if (data.data === 0) {
                        alert(`Promotion Size ${listSize[parseInt(item) - 1].size} Duplicate Date !!!`)
                        setCheck(false);
                        return;
                    }
                }).catch((err) => {
                    console.log("Sales Check Error :", err)
                })
            })
        } catch (error) {
            console.log(error)
    ...
    const schema = yup.object().shape({
        ...
        phanTramGiam: yup.number().required("Please Enter Discount Percentage")
    })
            ...
            <div id="content-wrapper" className="d-flex flex-column">
                <div id="content">
                    ...
                    <div className="container-fluid">
                        {/* Page Heading */}
                        <h1 className="h3 mb-2 text-gray-800">Promotion Manager</h1>
                        {/* DataTales Example */}
                            ...
                            <div className="card-header py-3" style={{ marginBottom: "5px" }}>
                                <h6 className="m-0 font-weight-bold text-primary" style={{ display: "flex", justifyContent: "space-between" }}>
                                    <Form.Select onChange={e => { setStatusSale(parseInt(e.target.value)); setpageSale(1) }} name="status" style={{ width: "300px" }}>
                                        ...
                                        <option value="1">Promotion in effect</option>
                                        <option value="2">Promotion Not Effective</option>
                                        <option value="-1">Promotion Expired</option>
                                    </Form.Select>
                                    <div className="col-lg-4 col-6 text-left" style={{ width: "30%" }}>
                                        <form>
                                            <div className="input-group">
                                                <input type="text" onChange={e => handleSearchSale(e.target.value)} className="form-control" placeholder="Search By Product Name" style={{ float: "right" }} />
                                    ...
                                    <Button variant="success" onClick={handleShow} style={{ top: "8px", right: "10px" }}>Add New Promotion</Button>
                                </h6>
                            </div>
                    ...
                    <Formik
                        onSubmit={async (values, { resetForm }) => {
                            ...
                            if (isNaN(values.maSP)) {
                                ...
                                console.log(arrSize)
                                alert("Please Choose At least 1 Size !!!");
                            } else {
                                        ...
                                        try {
                                            arrSize.forEach((item, index) => {
                                                axios({
                                                    ...
                                                    console.log("Error Adding Promotion :", err)
                                                })
                                            })
                                        } catch (error) {
                                            console.log(error)
                                        }
                                        alert("Add Successful Promotions !!!")
                    ...
                    >
                        {({
                            ...
                            <Form onSubmit={handleSubmit} style={{ color: "black", fontSize: "18px" }}>
                                    ...
                                    <Row>
                                        <Col>
                                            <div>
                                                ...
                                                <div className="input-group">
                                                    <input type="text" value={valueSearch} onChange={e => handleSearch(e.target.value)} className="form-control" placeholder="Search By Product Name" style={{ float: "right" }} />
                                                    <div className="input-group-append">
                                                        <span className="input-group-text bg-transparent text-primary">
                                                            <i className="fa fa-search" />
                                                        </span>
                                        ...
                                        <Col>
                                            <Form.Label>Product Name - Product Code: <strong>{infoProduct.tenSP} - {infoProduct.maSP}</strong> </Form.Label>
                                ...
                                <Button type="submit" variant="success" style={{ marginTop: "15px", float: "right" }} > Add Promotions</Button>
                            </Form>
                        )}
                    </Formik>
                </Modal.Body >
                                ...
                                if (check === true) {
                                    try {
                                        axios({
                                            ...
                                            console.log("Promotion Edit Error :", err)
                                        })
                                    } catch (error) {
                                        console.log(error)
                                    }
                                    alert("Edit Successful Promotion !!!")
                                    window.location.href = '/admin/sale'
                                }
                            } else {
                                alert("Expiration Date Must be After Start Date !!!")
                        ...
                        {({
                            ...
                            <Form onSubmit={handleSubmit} style={{ color: "black", fontSize: "18px" }}>
                                <Row>
                                    <Row>
                                        <Col>
                                            <Form.Label>Product Name - Product Code: <strong>{values.tenSP} - {values.maSP}</strong> </Form.Label>
                                ...
                                <Button type="submit" variant="success" style={{ marginTop: "15px", float: "right" }} > Edit Promotion</Button>
                            </Form>
                        )}
                    </Formik>
                </Modal.Body >
